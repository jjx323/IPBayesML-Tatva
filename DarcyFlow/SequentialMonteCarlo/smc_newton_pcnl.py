...
params = {
    ...
    "mean": lambda x: 0.0*np.sin(2*np.pi*(x[0] if hasattr(x, '__getitem__') else x))*np.cos(2*np.pi*(x[1] if hasattr(x, '__getitem__') and len(x)>1 else x)) if hasattr(x, '__call__') or hasattr(x, '__len__') else 0.0
}
...
num_particles = 500  ## particle number
...
for num in range(num_layers):
    ...
    ## ---------------------------------------------
    ## Set parameters of optimizer and eigensystem
    sampler = Newton_pCNL(model, dt=0.005, beta=0.01, reduce_chain=np.int64(100))
    ## For current program, sampler.mode should be "map_hessian"
    ...
    sampler.eigensystem_optims["method"] = "double_pass"
...
end_times = time.time()
start_times_list = comm.gather(start_times, root=0)
end_times_list = comm.gather(end_times, root=0)

if rank == 0:
    start_time = min(start_times_list)
    end_time = max(end_times_list)
    print("Elapsed time = %.5f seconds." % (end_time-start_time))

    ## Calculate the posterior mean function
    post_mean = np.mean(particles, axis=0)
    plot_fun2d(
        post_mean, coords=coords, elements=elements, nx=200, show=False, 
        path=results_folder / "post_mean_smc_newton_pcnl.svg", \
        grid_on=False, package="matplotlib"
    )
    np.save(results_folder/"samples_smc_newton_pcnl", particles)

    ## Save figures of the samples (存放到当前方法的 RESULTS 目录下)
    samples_folder = results_folder / "samples_smc_newton_pcnl"
    samples_folder.mkdir(exist_ok=True, parents=True)
    for idx, particle in enumerate(particles):
        plot_fun2d(
            particle, coords=coords, elements=elements, nx=200, show=False, 
            path=samples_folder / ("sample_" + str(idx) + ".svg"), 
            grid_on=False, package="matplotlib"
        )

    ## Calculate the relative error with background truth
    true_fun_vals = None
...
print(f"\nSMC sampling completed! Results saved to {results_folder}")
