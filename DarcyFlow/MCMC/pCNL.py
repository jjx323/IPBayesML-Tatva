...
len_chain = np.int64(100000)
sample_file = Path(results_folder/("pCNL_samples_" + str(noise_level)))
sample_file.mkdir(exist_ok=True, parents=True)
pcnl = pCNL(
    model, beta=0.002, reduce_chain=np.int64(10000), save_path=sample_file
)
figure_file = Path(results_folder/("pCNL_figures_" + str(noise_level)))
...
class CallBack(object):
    ...
    def callback_fun(self, params):
        ...
        if num % self.num_fre == 0:
            ...
            print('Phi = %4.4f' % self.phi(params[0][0]))

            # Plot current sample
            plot_fun2d(
                params[0][0], coords=self.coords, elements=self.elements,
                nx=200, show=False, 
                path=self.save_path/("fun_" + str(num) + ".svg"), 
                grid_on=False, package="matplotlib"
            )

callback = CallBack()

print("Starting pCNL sampling ......")
pcnl.sampling(len_chain=len_chain, callback=callback.callback_fun)

## Plot the trace of sampling function u
path_samples = sample_file
num_total = np.int64(len([f for f in os.listdir(path_samples) if f.endswith('.npy')]))
num_start = 0
trace_u = []
...
plt.figure()
plt.plot(trace_u)
plt.savefig(results_folder/"trace_pcnl.svg")
plt.close()
